...
interface ImageSlideOverDataProps {
  slideOverImage: NonNullable<SerializeFrom<ImagesLoader>["slideOverImage"]>;
}

function ImageSlideOverData({ slideOverImage }: ImageSlideOverDataProps) {
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-4">
          <h2 className="text-lg font-medium text-gray-900">
            Original image
            <span className="sr-only">Details for original image</span>
          </h2>
        ...
        >
          ...
          >
            ...
            <span className="sr-only">
              {slideOverImage.favorited ? "Unfavorite" : "Favorite"}
            </span>
          </button>
        </Form>
      ...
      <div>
        <h3 className="font-medium text-gray-900">Enhancement details</h3>
        <dl className="mt-2 divide-y divide-gray-200 border-t border-b border-gray-200">
          <div className="flex justify-between py-3 text-sm font-medium">
            <dt className="text-gray-500">Effect</dt>
            <dd className="text-gray-900">{slideOverImage.effect}</dd>
          </div>
          <div className="flex justify-between py-3 text-sm font-medium">
            <dt className="text-gray-500">Created</dt>
            <dd className="text-gray-900">
              <time>
                {format(new Date(slideOverImage.createdAt), "MMMM d, yyyy")}
              </time>
            </dd>
          </div>
          <div className="flex justify-between py-3 text-sm font-medium">
            ...
            <dd className="text-gray-900">
              {slideOverImage.timeMetric
                ? `${Math.round(slideOverImage.timeMetric)}s`
                : "Unknown"}
            </dd>
          </div>
          ...
          <div className="flex justify-between py-3 text-sm font-medium">
            ...
            <dd className="text-gray-900">{slideOverImage.results.length}</dd>
          </div>
        </dl>
      </div>
    </div>
              ...
              >
                <Dialog.Panel className="pointer-events-auto relative w-full max-w-[30rem]">
                  ...
                  <div className="h-full overflow-y-auto bg-white p-8">
                    {slideOverImage ? (
                      <ImageSlideOverData slideOverImage={slideOverImage} />
                    ) : (
                      <>
